...
static int synaptics_enable_interrupt_for_gesture(struct synaptics_ts_data *ts, int enable)
{
	...
	printk("F12_2D_CTRL20:0x%x=[2]:0x%x\n", F12_2D_CTRL20, reportbuf[2]);
...
static ssize_t double_tap_enable_write_func(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	...
	DouTap_gesture = ret;
	if (DouTap_gesture || Circle_gesture || UpVee_gesture || DownVee_gesture ||
...
static ssize_t letter_o_enable_write_func(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	...
	Circle_gesture = ret;
	if (DouTap_gesture || Circle_gesture || UpVee_gesture ||  DownVee_gesture ||
                (LeftVee_gesture && RightVee_gesture &&  DownVee_gesture && DouSwip_gesture))
	{
		ts->double_enable = 1;
		syna_use_gesture = 1;
	}
...
static ssize_t up_arrow_enable_read_func(struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
	int ret = 0;
	char page[PAGESIZE];
	ret = sprintf(page, "%d\n", DownVee_gesture);
...
static ssize_t up_arrow_enable_write_func(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	int ret = 0;
	struct synaptics_ts_data *ts = ts_g;
...
static const struct file_operations up_arrow_enable_proc_fops = {
	.write = up_arrow_enable_write_func,
	.read =  up_arrow_enable_read_func,
	.open = simple_open,
	.owner = THIS_MODULE,
};
...
TEST_WITH_CBC_s3508:
	...
	if(enable_cbc){
		...
		baseline_data_test = (int16_t *)cap_data_JDI[0];
		...
		ret = i2c_smbus_write_word_data(ts->client, F54_ANALOG_COMMAND_BASE, 0x04); // force update
		ret = i2c_smbus_write_byte_data(ts->client, F54_ANALOG_CONTROL_BASE+7, 0x01);// Forbid NoiseMitigation
		baseline_data_test = (int16_t *)cap_data_JDI[1];
	}
	/******write No Relax to 1******/
	ret = i2c_smbus_write_word_data(ts->client, F54_ANALOG_COMMAND_BASE, 0x04); // force update
	checkCMD();
...
static int synaptics_ts_suspend(struct device *dev)
{
    ...
    ret = synaptics_mode_change(0x01);
	if( ret < 0 ){
		TPD_ERR("%s: synaptics s3320 to sleep failed\n", __func__);
		return -1;
	}
	touch_disable(ts);
...
//#endif
	...
	if( ts->double_enable )
	{
		...
		if(ret<0)
			TPD_ERR("%s: failed to disable gesture report!!\n", __func__);
	}
    gpio_set_value(ts->reset_gpio,0);
    msleep(5);
    gpio_set_value(ts->reset_gpio,1);
